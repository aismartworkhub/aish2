...
export default function YoutubeAdvancedSearch({ youtubeApiKey, initialApply }: Props) {
  ...
  const applyHistory = (opts: SearchOptsSnapshot) => {
    ...
    // 결과 캐시 복원 (선호 채널 모드는 캐시 미적용)
    if (opts.useFavoritesOnly) {
      toast("폼 조건이 복원되었습니다. '검색' 버튼을 눌러 조회하세요.", "info");
      return;
    }
    const searchOpts = snapshotToYoutubeSearchOpts(opts);
    const cached = readSearchCache(searchOpts);
    if (!cached) {
      toast("결과 캐시가 만료되었습니다. '검색' 버튼을 눌러 새로 조회하세요.", "info");
      return;
    }
    const filtered = filterItemsByDurations(cached, opts.durations);
    setResults(filtered);
    setSearched(true);
    setFromCache(true);
    ...
    for (const v of items) {
      ...
      try {
        await createContent({
          ...
          channelUrl: v.channelId ? `https://www.youtube.com/channel/${v.channelId}` : undefined,
          publishedAtSource: v.publishedAt,
          viewCountSource: v.viewCount,
          likeCountSource: v.likeCount,
