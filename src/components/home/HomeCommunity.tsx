...
const MEMBER_BENEFITS = [
  ...
  { icon: Bell, title: "공지/일정 안내", desc: "모집, 마감, 운영 소식을 빠르게 확인.", href: "/community?tab=notice" },
  { icon: Handshake, title: "협력 문의", desc: "교육 위탁, 기업 제휴 등 협력을 제안.", href: "/community?tab=inquiry" },
];
        ...
        <div className="relative z-10 h-full flex items-center">
          <div className="ml-[8%] md:ml-[10%] max-w-[700px] text-white">
            ...
            <div className="mt-9 flex flex-wrap gap-4">
              ...
              <Link
                href="/media"
                className="inline-flex items-center gap-2 px-8 py-3.5 border border-white/40 text-white text-base font-medium rounded-sm uppercase tracking-widest hover:bg-white/10 transition-colors backdrop-blur-sm"
              >
                자료/콘텐츠 보기
                <ArrowRight size={18} />
              </Link>
          ...
          <div className="flex-1 bg-white p-8 md:p-11">
            ...
            <div className="flex items-center border-b-2 border-gray-300 pb-1">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && searchTerm.trim()) router.push(`/programs?q=${encodeURIComponent(searchTerm.trim())}`);
                }}
              />
              <button className="text-gray-500 hover:text-brand-blue transition-colors" onClick={() => { if (searchTerm.trim()) router.push(`/programs?q=${encodeURIComponent(searchTerm.trim())}`); }}>
                <Search size={22} />
              </button>
            </div>
          </div>
          <div className="flex-1 bg-brand-blue p-8 md:p-11 text-white">
            ...
            <div className="flex items-center border-b-2 border-white/40 pb-1">
              <select
                ...
                onChange={(e) => { if (e.target.value) router.push(`/programs?category=${encodeURIComponent(e.target.value)}`); }}
                defaultValue=""
              >
        ...
        <div className={cn("max-w-5xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6")}>
          {instructors.map((ins) => (
            <Link key={ins.id} href="/instructors" ref={addRevealRef}
              className={cn("bg-white rounded-sm border border-brand-border shadow-sm overflow-hidden group", "cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all duration-300")}>
              <div className={cn("relative aspect-[4/5] overflow-hidden bg-gradient-to-br from-brand-gray to-gray-200")}>
                {(ins.imageUrl || ins.profileImageUrl) ? (
                  <DriveOrExternalImage src={(ins.imageUrl || ins.profileImageUrl)!} alt={ins.name} className={cn("w-full h-full object-cover object-top")} quiet />
                ) : (
              ...
              <div className={cn("px-6 py-4")}>
                <div className={cn("mb-3 min-w-0")}>
                  <h3 className={cn("text-xl font-bold text-gray-900 tracking-tighter leading-tight")}>{ins.name}</h3>
                  {ins.title && <p className={cn("mt-0.5 text-sm font-bold text-brand-blue tracking-tighter line-clamp-2 break-keep")}>{ins.title}</p>}
                </div>
                ...
                {ins.bio && (
                  <div className={cn("bg-brand-gray/60 rounded-xl px-3.5 py-2.5 max-h-[5rem] overflow-hidden")}>
                    <p className={cn("text-[13px] text-gray-600 italic leading-snug text-center tracking-tighter font-medium", "line-clamp-3 break-keep")}>
                      &ldquo;{ins.bio}&rdquo;
                    </p>
                  </div>
                )}
          ...
          <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {MEMBER_BENEFITS.map((item) => (
              <Link
                ...
                ref={addRevealRef}
                className={cn(
                  "group bg-white rounded-sm border border-brand-border p-8",
                  "hover:shadow-lg hover:border-brand-blue/30 hover:-translate-y-1 transition-all duration-300",
                )}
              >
                <div className="w-14 h-14 rounded-sm bg-brand-blue/10 flex items-center justify-center mb-5">
                ...
                <span className="mt-4 inline-flex items-center gap-1 text-xs text-brand-blue font-medium group-hover:underline">
        ...
        <div className="container-custom">
          ...
          <div className="w-[90%] max-w-[1200px] mx-auto flex flex-col md:flex-row justify-center gap-6">
            {specialtyCardsResolved.map((card) => (
              <div key={card.subtitle} ref={addRevealRef} className="flex-1 rounded overflow-hidden bg-white border border-brand-border shadow-[0_10px_30px_rgba(0,0,0,0.05)]">
      ...
      <section className="flex flex-col md:flex-row min-h-[550px]">
        <div className="flex-1 relative flex items-center px-[6%] md:px-[8%] py-16 text-white overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <Link href="/workathon" className="mt-7 inline-flex items-center justify-center w-[60px] h-[60px] rounded-full bg-white text-brand-blue text-2xl font-bold hover:bg-brand-blue hover:text-white hover:-rotate-45 transition-all duration-300">
              &#10148;
            </Link>
          </div>
        </div>
          ...
          <div className="container-custom">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {adminEvents.slice(0, 6).map((evt) => (
                <div key={evt.id} ref={addRevealRef} className="bg-white rounded-sm overflow-hidden border border-brand-border hover:shadow-lg hover:border-t-4 hover:border-t-brand-blue transition-shadow">
                  {evt.thumbnailUrl ? (
                    <div className="h-40 overflow-hidden"><img src={evt.thumbnailUrl} alt={evt.title} className="w-full h-full object-cover" loading="lazy" /></div>
                  ) : (
                    <div className="h-40 bg-gradient-to-br from-brand-gray to-blue-50 flex items-center justify-center"><Trophy size={36} className="text-brand-lightBlue" /></div>
                  )}
                  <div className="p-5">
        ...
        <div className="flex-1 bg-[#1a1a2e] p-12 md:p-16 flex flex-col justify-center text-white">
          ...
          <div className="flex flex-wrap gap-4">
            <a href={ctaCfg.buttonUrl} target={isExternalHref(ctaCfg.buttonUrl) ? "_blank" : undefined} rel={isExternalHref(ctaCfg.buttonUrl) ? "noopener noreferrer" : undefined}
              className="inline-flex items-center gap-2 px-6 py-3 bg-white text-brand-blue text-sm font-bold rounded-sm hover:bg-brand-blue hover:text-white transition-all">
              {ctaCfg.buttonText} <ArrowRight size={16} />
            </a>
            <Link href="/community" className="inline-flex items-center gap-2 px-6 py-3 border border-white/30 text-white text-sm font-bold rounded-sm hover:bg-white/10 transition-all">
              커뮤니티 참여하기 <ArrowRight size={16} />
            </Link>
          </div>
        </div>
        )}
